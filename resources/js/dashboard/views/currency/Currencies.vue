                ...
                <vs-tr :key="indexItem" v-for="(item, indexItem) in data" >
                    ...
                    <vs-td>
                        <div class="w-full flex">
                            <vs-button color="primary" class="mr-2" icon="edit" type="filled" :to="{ name: instanceSlug + '_edit', params: {id: item.id} }"></vs-button>
                            <vs-button color="danger" icon="delete" type="filled" @click="showDeleteAlert(item.id)" v-if="items.length > 1"></vs-button>
                        </div>
                    </vs-td>
                </vs-tr>
            </template>
...
<script>
    ...
    import { tableSchema } from "@/static/currency/table";
