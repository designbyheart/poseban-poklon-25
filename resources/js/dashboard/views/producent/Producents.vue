            ...
            <template slot="header">
                <div class="vx-row w-full pb-3 m-0">
                    <div class="vx-col flex items-center w-1/2 p-0">
                        ...
                        <vs-button color="primary" type="filled" :to="{ name: 'producents_create' }">{{ $t('actions.create') }}</vs-button>
                    </div>
            ...
            <template slot-scope="{data}">
                <vs-tr :key="indexItem" v-for="(item, indexItem) in data" >
                    <vs-td :data="item[column.model]" :key="index" v-for="(column, index) in tableSchema.columns">
                        <span v-if="column.isRelation">{{ item[column.model][column.relationProperty] }}</span>
                        <span v-else>{{ item[column.model] }}</span>
                    </vs-td>

                    <vs-td>
                        <div class="w-full flex">
                            <vs-button color="primary" class="mr-2" type="filled" :to="{ name: 'producents_edit', params: {id: item.id} }">{{ $t('actions.edit') }}</vs-button>
...
<script>
    ...
    import { tableSchema } from "@/static/producent/table";
