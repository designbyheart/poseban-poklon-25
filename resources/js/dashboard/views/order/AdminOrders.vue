        ...
        <vs-table :max-items="per_page" :sst="true" :data="items" :noDataText="$t('messages.notFound')" @sort="handleSort">
            ...
            <template slot-scope="{data}">
                <vs-tr :key="indexItem" v-for="(order, indexItem) in data" >
                    <vs-td>
                        <span class="text-primary">#{{order.id}}</span> <span class="text-sm">{{order.customer_name + ' ' + order.customer_surname}}</span>
                        <br/>
                    ...
                    <vs-td>
                        <vs-button color="primary" class="text-sm" :to="{ name: instanceSlug + '_show', params: {id: order.id} }">{{ $t('actions.view') }}</vs-button>
                    </vs-td>
                </vs-tr>
            </template>
